...
(function ($) {
  ...
  jQuery(elem).each((a, b) => {
    const id = jQuery(b).attr('id');
    new Vue({
      el        : '#' + id,
      data      : function () {
        return {
      ...
      methods   : {
        ...
        startSearch() {
          const searchText   = this.searchKeyword.toString().trim().toLowerCase();
          const results      = this.rawResults.filter((e) => {
          ...
          // console.log({results,returnResult});
        },
        startSearch2() {
          setTimeout(() => {
            this.xhrStartSearch();
        ...
        addEvents3() {
          window.onscroll = () => {
            ...
            this.resultTopOffset = topOffset;
      ...
      `;
        ...
        xhrLoadEndpoints() {
          let dis                      = this;
          const handleAjax: HandleAjax = new HandleAjax(this.zajax.default);
          new Server().send_online({
            data: [
              this.localize.nonce,
              {
                params: {
                  per_page      : this.tableData.paginationOptions.perPage,
                  page          : this.tableData.paginationOptions.setCurrentPage,
                  search_keyword: this.searchKeyword,
                  status        : this.tableData.post_status,
                },
              }
            ],
            ...
            funcSuccess(done: InterFuncSuccess) {
              ...
              dis.rawResults = done.data.details.endpoints;
            },
            funcFailue(done) {
        ...
        xhrStartSearch() {
          ...
          new Server().send_online({
            ...
            funcSuccess(done: InterFuncSuccess) {
              handleAjax.success(done);
              dis.results = done.data.details.endpoints;
            },
            funcFailue(done) {
              handleAjax.error(done);
              // this.tableData.isLoading = false;
        ...
        root_pick_image_from_media(select_button_text: any, select_head_text: any, function_gotten: any) {
           ...
           * this.pick_image_from_media("Pick", "Pick Background Image", function (result) {
                dis.reset();
                if (this.html_data.only_as_default) {
                    this.html_data_default.image = result.url;

                } else {
                    this.html_data.image = result.url;
                }
            });
      ...
      computed  : {
        searchButtonLink(): string {
          return this.localize.site_url + '/' + this.searchKeyword;
        },
        ajax() {
          return this.zajax
        },
