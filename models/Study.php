...
class Study extends Model {
	...
	public static function get_user_studies( $args ): array {
		...
		if ( $args['with_trashed'] ) {
			$studies = self::withoutTrashed();
		} elseif ( $args['only_trashed'] ) {
			$studies = self::onlyTrashed();
		} else {
			$studies = self::where( 'id', '>', 0 );
		}
		$studies->with(
			array(
				'tags',
	...
	public static function get_user_study_by_id( $study_id ) {
		return self::with( 'tags', 'deck' )->find( $study_id );
	}
	...
	public static function get_user_stats_card_types( $user_id, $span ) {
		...
		$graphable['pie_data2']   = array( $total_matured, $total_young, $total_new_cards );
		...
		foreach ( $card_groups as $_card_group ) {
			...
			foreach ( $all_new_cards as $key => $new_card ) {
				...
				if ( ! array_key_exists( $study_id, $hold_studies ) ) {
					$hold_studies[ $study_id ] = array();
				}
				if ( ! array_key_exists( $_new_day_index, $hold_studies[ $study_id ] ) ) {
					$hold_studies[ $study_id ][ $_new_day_index ] = array(
						'count' => 0,
					);
				}
...
//		] );
		...
		foreach ( $forecast_all_answers_within_a_date as $answer ) {
			...
			$days[ $_key ][ $grade ] ++;
			$days[ $_key ]['total'] ++;
			if ( ! in_array( $answer->grade, array( 'again', 'hold' ) ) ) {
	...
	public static function get_user_stats_charts_added( $user_id, $span ) {
		...
		$graphable                         = array(
			'heading'                   => array(),
			'new_cards_added'           => array(),
			'cumulative_new_cards'      => array(),
			'total_new_cards'           => 0,
	...
	public static function get_user_stats_charts_intervals( $user_id, $span ) {
		...
		if ( empty( $graphable['heading'] ) ) {
			$graphable['heading'] = array( '1d', '2d', '3d', '4d', '5d', '6d', '7d' );
		}
	...
	public static function get_user_card_review_count_and_time( $user_id, $span ) {
		...
		$graphable                         = array(
			...
			'y_time'                        => array(),
			'm_time'                        => array(),
			'newly_learned_time'            => array(),
			'relearned_time'                => array(),
			'cumulative_y'                  => array(),
			'cumulative_m'                  => array(),
			'cumulative_newly_learned'      => array(),
			'cumulative_relearned'          => array(),
			'cumulative_y_time'             => array(),
			'cumulative_m_time'             => array(),
			'cumulative_newly_learned_time' => array(),
			'cumulative_relearned_time'     => array(),
			'y_debug'                       => array(
				'answers'   => array(),
				'new_cards' => array(),
			),
			...
			'newly_learned_debug'           => array(
				'answers'   => array(),
				'new_cards' => array(),
			),
			'relearned_debug'               => array(
				'answers'   => array(),
				'new_cards' => array(),
			),
			'm_cumulative'                  => 0,
		...
		self::add_debug(
			'Average for days studied (minutes)',
			array(
		...
		self::add_debug(
			'If you studied every day (average) (minutes)',
			array(
		...
		$formed_total_time       = $total_time_hours < 1.0
			? ( $total_time_minutes < 1.0 ? number_format( $total_time_seconds, 2 ) . ' seconds'
				: number_format( $total_time_minutes, 2 ) . ' minutes' )
			: number_format( $total_time_hours, 2 ) . ' hours';
	...
	public static function get_forecast_cards_on_hold( $args ) {
		...
		foreach ( $user_studies as $key => $study ) {
			...
			if ( $args['no_date_limit'] ) {
				...
				$query_answer = $query_answer->whereBetween(
					'next_due_at',
					array(
						$args['start_date'],
						$args['end_date'],
	...
	public static function get_user_cards_____( $study_id, $user_id ) {
		...
		try {
			...
			$study             = self::with( 'tags', 'tags_excluded' )->find( $study_id );
	...
	public static function get_user_cards_on_hold( $study_id, $user_id, $particular_date = null ) {
		...
		try {
			...
			$study         = self::with( 'tags', 'tags_excluded' )->findOrFail( $study_id );
			$deck_id       = $study->deck_id;
			$tags          = array();
			$tags_excluded = array();
			$add_all_tags  = $study->all_tags;
	...
	/**
	 * Returns the new cards the user needs to study today.
	 *
	...
	public static function get_user_cards_new( $study_id, $user_id ): array {
		...
		try {
			...
			// If does not want to study all tags, get the specific cards he want and those he doesnt want.
			if ( ! $add_all_tags ) {
				$tags          = $study->tags->pluck( 'id' );
				$tags_excluded = $study->tagsExcluded->pluck( 'id' );
			}
	...
	public static function get_user_cards_to_study( $study_id, $user_id ) {
		...
		$user_cards_new     = self::get_user_cards_new( $study_id, $user_id );
		$user_cards_revise  = self::get_user_cards_to_revise( $study_id, $user_id );
		$user_cards_on_hold = self::get_user_cards_on_hold( $study_id, $user_id );
