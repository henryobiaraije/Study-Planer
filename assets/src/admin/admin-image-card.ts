...
import "../../css/admin/admin-image-card.scss";
...
const mComCard = {
  ...
  imageCardItem() {
    return dis(this).useImageCard.imageItem.value;
  },
  imageCardGroup() {
    return dis(this).useImageCard.cardGroup.value;
  },
};
...
const m_init = {
  init() {
    ...
    dis(this).useImageCard._load().then(() => {
      jQuery('.all-loading').hide();
      jQuery('.all-loaded').show();
      vdata.showMain = true;
      this.generalInit();
      setTimeout(() => {
        dis(this).useImageCard._addEvents();
      }, 1000);
    }).catch(() => {
      jQuery('.all-loading').hide();
