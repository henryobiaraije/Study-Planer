...
//		                      ->forceDelete();
	...
	public function admin_update_basic_card( $post ): void {
		...
		// If its a reverse card, update the reverse version or
		$old_reverse_card = Card
			::query()
			->where( 'card_group_id', '=', $card_group->id )
			->where( 'c_number', '=', 'c2' )
			->first();

		if ( $reverse ) {
			if ( $old_reverse_card ) {
				$old_reverse_card->question = $answer;
				$old_reverse_card->answer   = $question;
				$old_reverse_card->save();
			} else {
				$card                = new Card();
	...
	public function ajax_admin_create_new_basic_card( $post ): void {
		...
		// Sync cg.deck_id to cg.topic.deck_id
